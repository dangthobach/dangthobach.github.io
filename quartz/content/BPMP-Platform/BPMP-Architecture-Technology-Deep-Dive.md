...
flowchart TB
    subgraph Edge["Edge and Experience"]
        WEB["Cockpit Web\nReact 19 + Vite + Nginx"]
        API["API Gateway\nGo REST / gRPC client"]
        PUSH["Cockpit Gateway\nGo SSE / realtime"]
        AUTHAPP["Authz Administration API\nRust"]
    end

    subgraph Control["Control Plane"]
        CFG["Configuration Service\nGo"]
        AUTHSRV["Authz Policy Server\nRust + PostgreSQL"]
        GOV["Governance Service\nGo/Rust bounded context"]
        COMP["BPMN Compiler\nRust"]
    end
    ...
    subgraph Execution["Execution and Query"]
        ...
        WASM["Local WASM Worker\nWasmtime"]
        REMOTE["Remote Workers\nbidirectional tonic gRPC"]
    end

    subgraph Platform["Platform Services"]
        KAFKA["Kafka / Redpanda\nIntegration Feed"]
        REDIS["Redis\nDistributed rate limit / ephemeral cache"]
...
```mermaid
flowchart LR
    ROUTER["Shard Directory"] --> G1["Raft Group A\n3 Engine nodes"]
    ROUTER --> G2["Raft Group B\n3 Engine nodes"]
    ROUTER --> G3["Raft Group C\n3 Engine nodes"]
