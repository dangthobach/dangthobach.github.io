---
tags: [moc, system-design, reading-list]
type: moc
domain: knowledge-management
status: active
created: 2026-04-12
updated: 2026-04-12
---

# 📚 Reading List MOC — System Design & Architecture

Archive từ Notion — các bài newsletter/article về system design. Dùng như **reference library**, không phải evergreen notes.

> Workflow: Khi đọc xong 1 bài và có insight → tạo note riêng trong `concepts/` với template-concept, link ngược lại đây.

---

## 🏗️ Architecture & Design Principles
- [[Notion Knowledge/Note/Clean Architecture 101- Building Software That Lasts|Clean Architecture 101]]
- [[Notion Knowledge/Note/Software Architecture Patterns|Software Architecture Patterns]]
- [[Notion Knowledge/Note/Coupling and Cohesion- The Two Principles for Effective Architecture|Coupling and Cohesion]]
- [[Notion Knowledge/Note/Mastering OOP Fundamentals with SOLID Principles|Mastering OOP with SOLID]]
- [[Notion Knowledge/Note/What is the SOLID Principle|What is the SOLID Principle]]
- [[Notion Knowledge/Note/OOP Design Patterns and Anti-Patterns- What Works and What Fails|OOP Design Patterns & Anti-Patterns]]
- [[Notion Knowledge/Note/Monolith vs Microservices vs Modular Monoliths- What's the Right Choice|Monolith vs Microservices vs Modular Monoliths]]
- [[Notion Knowledge/Note/Tidying Code|Tidying Code]]
- [[Notion Knowledge/Note/Non-Functional Requirements- The Backbone of Great Software - Part 1|NFR Part 1]]
- [[Notion Knowledge/Note/Non-Functional Requirements- The Backbone of Great Software - Part 2|NFR Part 2]]
- [[Notion Knowledge/Note/Domain-Driven Design (DDD) Demystified|DDD Demystified]]
- [[Notion Knowledge/Note/A Crash Course on Domain-Driven Design|DDD Crash Course]]

---

## 🌐 Distributed Systems
- [[Notion Knowledge/Note/A Crash Course on Distributed Systems|Distributed Systems Crash Course]]
- [[Notion Knowledge/Note/CAP, PACELC, ACID, BASE - Essential Concepts for an Architect’s Toolkit|CAP, PACELC, ACID, BASE]]
- [[Notion Knowledge/Note/Consistency and Partition Tolerance- Understanding CAP vs PACELC|CAP vs PACELC]]
- [[Notion Knowledge/Note/Dark Side of Distributed Systems- Latency and Partition Tolerance|Dark Side of Distributed Systems]]
- [[Notion Knowledge/Note/Engineering Trade-offs- Eventual Consistency in Practice|Eventual Consistency in Practice]]
- [[Notion Knowledge/Note/Mastering Data Consistency Across Microservices|Data Consistency Across Microservices]]
- [[Notion Knowledge/Note/Top Leader Election Algorithms in Distributed Databases|Leader Election Algorithms]]
- [[Notion Knowledge/Note/Top Strategies to Improve Reliability in Distributed Systems|Reliability Strategies]]
- [[Notion Knowledge/Note/Embracing Chaos to Improve System Resilience- Chaos Engineering|Chaos Engineering]]
- [[Notion Knowledge/Note/Consistent Hashing 101- How Modern Systems Handle Growth and Failure|Consistent Hashing 101]]

---

## 🔗 Microservices
- [[Notion Knowledge/Note/A Crash Course on Microservices Design Patterns|Microservices Design Patterns]]
- [[Notion Knowledge/Note/A Crash Course on Microservice Communication Patterns|Microservice Communication Patterns]]
- [[Notion Knowledge/Note/Data Sharing Between Microservices|Data Sharing Between Microservices]]
- [[Notion Knowledge/Note/A Pattern Every Modern Developer Should Know- CQRS|CQRS]]
- [[Notion Knowledge/Note/The Saga Pattern|The Saga Pattern]]
- [[Notion Knowledge/Note/The Sidecar Pattern Explained- Decoupling Operational Features|Sidecar Pattern]]
- [[Notion Knowledge/Note/Event-Driven Architectural Patterns|Event-Driven Patterns]]
- [[Notion Knowledge/Note/Messaging Patterns Explained- Pub-Sub, Queues, and Event Streams|Messaging Patterns]]
- [[Notion Knowledge/Note/Synchronous vs Asynchronous Communication- When to Use What|Sync vs Async Communication]]
- [[Notion Knowledge/Note/Error Handling Patterns|Error Handling Patterns]]
- [[Notion Knowledge/Note/Mastering Idempotency- Building Reliable APIs|Mastering Idempotency]]

---

## 📡 API Design
- [[Notion Knowledge/Note/Best Practices in API Design|API Design Best Practices]]
- [[Notion Knowledge/Note/Mastering the Art of API Design|Mastering API Design]]
- [[Notion Knowledge/Note/The Art of REST API Design- Idempotency, Pagination, and Security|REST API Design]]
- [[Notion Knowledge/Note/A Crash Course in API Versioning Strategies|API Versioning Strategies]]
- [[Notion Knowledge/Note/API Security Best Practices|API Security Best Practices]]
- [[Notion Knowledge/Note/API Protocols 101- A Guide to Choose the Right One|API Protocols 101]]
- [[Notion Knowledge/Note/SOAP vs REST vs GraphQL vs RPC|SOAP vs REST vs GraphQL vs RPC]]
- [[Notion Knowledge/Note/A Crash Course in GraphQL|GraphQL Crash Course]]
- [[Notion Knowledge/Note/GraphQL 101- API Approach Beyond REST|GraphQL 101]]
- [[Notion Knowledge/Note/API Gateway|API Gateway]]
- [[Notion Knowledge/Note/API Gateway vs Service Mesh - Which One Do You Need|API Gateway vs Service Mesh]]

---

## 🗄️ Database
- [[Notion Knowledge/Note/A Crash Course in Database Scaling Strategies|DB Scaling Strategies]]
- [[Notion Knowledge/Note/A Crash Course in Database Sharding|DB Sharding Crash Course]]
- [[Notion Knowledge/Note/A Guide to Database Sharding- Key Strategies|DB Sharding Guide]]
- [[Notion Knowledge/Note/A Guide to Database Replication- Key Concepts and Strategies|DB Replication Guide]]
- [[Notion Knowledge/Note/Data Replication- A Key Component for Building Large-Scale Distributed Systems|Data Replication]]
- [[Notion Knowledge/Note/How to Choose a Replication Strategy|Replication Strategy]]
- [[Notion Knowledge/Note/A Guide to Database Transactions- From ACID to Concurrency Control|DB Transactions: ACID to Concurrency]]
- [[Notion Knowledge/Note/Database Index Internals- Understanding the Data Structures|DB Index Internals]]
- [[Notion Knowledge/Note/Database Indexing Demystified- Index Types and Use-Cases|DB Indexing Demystified]]
- [[Notion Knowledge/Note/Database Performance Demystified- Essential Tips and Strategies|DB Performance]]
- [[Notion Knowledge/Note/Database Schema Design Simplified- Normalization vs Denormalization|Schema Design: Normalization vs Denormalization]]
- [[Notion Knowledge/Note/SQL vs NoSQL- Choosing the Right Database for An Application|SQL vs NoSQL]]
- [[Notion Knowledge/Note/Key Steps in the Database Selection Process|DB Selection Process]]
- [[Notion Knowledge/Note/Understanding Database Types|Understanding DB Types]]
- [[Notion Knowledge/Note/Unlocking the Power of SQL Queries for Improved Performance|SQL Query Performance]]
- [[Notion Knowledge/Note/A Crash Course on Relational Database Design|Relational DB Design]]
- [[Notion Knowledge/Note/Material view|Materialized Views]]

---

## ⚡ Scalability & Performance
- [[Notion Knowledge/Note/A Crash Course on Architectural Scalability|Architectural Scalability]]
- [[Notion Knowledge/Note/A Crash Course on Scaling the API Layer|Scaling the API Layer]]
- [[Notion Knowledge/Note/A Crash Course on Scaling the Data Layer|Scaling the Data Layer]]
- [[Notion Knowledge/Note/Top Scalability Strategies for Real-World Load|Top Scalability Strategies]]
- [[Notion Knowledge/Note/A Crash Course on Load Balancers for Scaling|Load Balancers for Scaling]]
- [[Notion Knowledge/Note/Understanding Load Balancers- Traffic Management at Scale|Load Balancers Deep Dive]]
- [[Notion Knowledge/Note/A Detailed Guide to Content Delivery Networks|CDN Guide]]
- [[Notion Knowledge/Note/A Guide to Rate Limiting Strategies|Rate Limiting Strategies]]
- [[Notion Knowledge/Note/A Guide to Top Caching Strategies|Caching Strategies]]
- [[Notion Knowledge/Note/Distributed Caching- The Secret to High-Performance Applications|Distributed Caching]]
- [[Notion Knowledge/Note/Stateless Architecture- The Key to Building Scalable and Resilient Systems|Stateless Architecture]]

---

## 🔐 Auth & Security
- [[Notion Knowledge/Note/Mastering Modern Authentication- Cookies, Sessions, JWT, and PASETO|Modern Authentication]]
- [[Notion Knowledge/Note/JWT vs PASETO- The Two Players of Token-Based Authentication|JWT vs PASETO]]
- [[Notion Knowledge/Note/EP176- How Does SSO Work|How Does SSO Work]]
- [[Notion Knowledge/Note/Oauth2|OAuth2]]
- [[Notion Knowledge/Note/How Grab Built An Authentication System for 180+ Million Users|Grab Auth System]]

---

## 🌍 Networking & Infrastructure
- [[Notion Knowledge/Note/A Crash Course in Networking|Networking Crash Course]]
- [[Notion Knowledge/Note/A Crash Course in IPv4 Addressing|IPv4 Addressing]]
- [[Notion Knowledge/Note/A Deep Dive into HTTP- From HTTP 1 to HTTP 3|HTTP 1→3 Deep Dive]]
- [[Notion Knowledge/Note/HTTP1 vs HTTP2 vs HTTP3 - A Deep Dive|HTTP Versions Comparison]]
- [[Notion Knowledge/Note/Infrastructure as Code|Infrastructure as Code]]
- Kubernetes Roadmap *(planned)*
- [[Notion Knowledge/Note/What are the differences between Virtualization (VMware) and Containerization (Docker)|Virtualization vs Containerization]]
- [[Notion Knowledge/Note/A Crash Course on Cell-based Architecture|Cell-based Architecture]]
- [[Notion Knowledge/Note/Why is a solid-state drive (SSD) fast|Why is SSD Fast]]

---

## 🏢 Case Studies (Real Systems)
- [[Notion Knowledge/Note/How Atlassian Migrated 4 Million Jira Databases to AWS Aurora|Atlassian: 4M Jira DBs → Aurora]]
- [[Notion Knowledge/Note/How Dropbox Built an AI Product Dash with RAG and AI Agents|Dropbox: AI Dash with RAG]]
- [[Notion Knowledge/Note/How Grab Built An Authentication System for 180+ Million Users|Grab: Auth at Scale]]
- [[Notion Knowledge/Note/How LinkedIn Scaled User Restriction System to 5 Million Queries Per Second|LinkedIn: 5M QPS Restriction]]
- [[Notion Knowledge/Note/How Reddit Delivers Notifications to Tens of Millions of Users|Reddit: Notifications at Scale]]
- [[Notion Knowledge/Note/How Slack Supports Billions of Daily Messages|Slack: Billions of Messages]]
- [[Notion Knowledge/Note/How Tinder’s API Gateway Handles A Billion Swipes Per Day|Tinder: 1B Swipes/Day]]
- [[Notion Knowledge/Note/How Uber Eats Deduplicates Hundreds of Millions of Product Images|Uber Eats: Image Dedup]]
- [[Notion Knowledge/Note/How to avoid crawling duplicate URLs at Google scale|Google: URL Dedup at Scale]]
- [[Notion Knowledge/Note/Shopify Tech Stack|Shopify Tech Stack]]
- [[Notion Knowledge/Note/EP177- The Modern Software Stack|The Modern Software Stack]]

---

## 🔀 Concurrency
- [[Notion Knowledge/Note/Concurrency is NOT Parallelism|Concurrency is NOT Parallelism]]
- [[Notion Knowledge/Note/Understanding Message Queues|Understanding Message Queues]]
- [[Notion Knowledge/Note/Apache arvo, Protobuf|Apache Avro & Protobuf]]

---

## 💳 Domain-specific
- [[Notion Knowledge/Note/The Payments Ecosystem|The Payments Ecosystem]]
- [[Notion Knowledge/Note/Does Serverless Have Servers|Does Serverless Have Servers]]
- [[Notion Knowledge/Note/Why Executives Seem Out of Touch, and How to Reach Them|Why Executives Seem Out of Touch]]

---

## 🔗 Links vào hệ thống MOC
- [[MOC-System-Design]]
- [[MOC-Distributed-Systems]]
- [[MOC-Database]]
- [[MOC-Concurrency]]
