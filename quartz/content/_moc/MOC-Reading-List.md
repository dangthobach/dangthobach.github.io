---
tags: [moc, system-design, reading-list]
---

# 📚 Reading List MOC — System Design & Architecture

Archive từ Notion — các bài newsletter/article về system design. Dùng như **reference library**, không phải evergreen notes.

> Workflow: Khi đọc xong 1 bài và có insight → tạo note riêng trong `concepts/` với template-concept, link ngược lại đây.

---

## 🏗️ Architecture & Design Principles
- [[Clean Architecture 101- Building Software That Lasts|Clean Architecture 101]]
- [[Software Architecture Patterns|Software Architecture Patterns]]
- [[Coupling and Cohesion- The Two Principles for Effective Architecture|Coupling and Cohesion]]
- [[Mastering OOP Fundamentals with SOLID Principles|Mastering OOP with SOLID]]
- [[What is the SOLID Principle|What is the SOLID Principle]]
- [[OOP Design Patterns and Anti-Patterns- What Works and What Fails|OOP Design Patterns & Anti-Patterns]]
- [[Monolith vs Microservices vs Modular Monoliths- What's the Right Choice|Monolith vs Microservices vs Modular Monoliths]]
- [[Tidying Code|Tidying Code]]
- [[Non-Functional Requirements- The Backbone of Great Software - Part 1|NFR Part 1]]
- [[Non-Functional Requirements- The Backbone of Great Software - Part 2|NFR Part 2]]
- [[Domain-Driven Design (DDD) Demystified|DDD Demystified]]
- [[A Crash Course on Domain-Driven Design|DDD Crash Course]]

---

## 🌐 Distributed Systems
- [[A Crash Course on Distributed Systems|Distributed Systems Crash Course]]
- [[CAP, PACELC, ACID, BASE - Essential Concepts for an Architect's Toolkit|CAP, PACELC, ACID, BASE]]
- [[Consistency and Partition Tolerance- Understanding CAP vs PACELC|CAP vs PACELC]]
- [[Dark Side of Distributed Systems- Latency and Partition Tolerance|Dark Side of Distributed Systems]]
- [[Engineering Trade-offs- Eventual Consistency in Practice|Eventual Consistency in Practice]]
- [[Mastering Data Consistency Across Microservices|Data Consistency Across Microservices]]
- [[Top Leader Election Algorithms in Distributed Databases|Leader Election Algorithms]]
- [[Top Strategies to Improve Reliability in Distributed Systems|Reliability Strategies]]
- [[Embracing Chaos to Improve System Resilience- Chaos Engineering|Chaos Engineering]]
- [[Consistent Hashing 101- How Modern Systems Handle Growth and Failure|Consistent Hashing 101]]

---

## 🔗 Microservices
- [[A Crash Course on Microservices Design Patterns|Microservices Design Patterns]]
- [[A Crash Course on Microservice Communication Patterns|Microservice Communication Patterns]]
- [[Data Sharing Between Microservices|Data Sharing Between Microservices]]
- [[A Pattern Every Modern Developer Should Know- CQRS|CQRS]]
- [[The Saga Pattern|The Saga Pattern]]
- [[The Sidecar Pattern Explained- Decoupling Operational Features|Sidecar Pattern]]
- [[Event-Driven Architectural Patterns|Event-Driven Patterns]]
- [[Messaging Patterns Explained- Pub-Sub, Queues, and Event Streams|Messaging Patterns]]
- [[Synchronous vs Asynchronous Communication- When to Use What|Sync vs Async Communication]]
- [[Error Handling Patterns|Error Handling Patterns]]
- [[Mastering Idempotency- Building Reliable APIs|Mastering Idempotency]]

---

## 📡 API Design
- [[Best Practices in API Design|API Design Best Practices]]
- [[Mastering the Art of API Design|Mastering API Design]]
- [[The Art of REST API Design- Idempotency, Pagination, and Security|REST API Design]]
- [[A Crash Course in API Versioning Strategies|API Versioning Strategies]]
- [[API Security Best Practices|API Security Best Practices]]
- [[API Protocols 101- A Guide to Choose the Right One|API Protocols 101]]
- [[SOAP vs REST vs GraphQL vs RPC|SOAP vs REST vs GraphQL vs RPC]]
- [[A Crash Course in GraphQL|GraphQL Crash Course]]
- [[GraphQL 101- API Approach Beyond REST|GraphQL 101]]
- [[API Gateway|API Gateway]]
- [[API Gateway vs Service Mesh - Which One Do You Need|API Gateway vs Service Mesh]]

---

## 🗄️ Database
- [[A Crash Course in Database Scaling Strategies|DB Scaling Strategies]]
- [[A Crash Course in Database Sharding|DB Sharding Crash Course]]
- [[A Guide to Database Sharding- Key Strategies|DB Sharding Guide]]
- [[A Guide to Database Replication- Key Concepts and Strategies|DB Replication Guide]]
- [[Data Replication- A Key Component for Building Large-Scale Distributed Systems|Data Replication]]
- [[How to Choose a Replication Strategy|Replication Strategy]]
- [[A Guide to Database Transactions- From ACID to Concurrency Control|DB Transactions: ACID to Concurrency]]
- [[Database Index Internals- Understanding the Data Structures|DB Index Internals]]
- [[Database Indexing Demystified- Index Types and Use-Cases|DB Indexing Demystified]]
- [[Database Performance Demystified- Essential Tips and Strategies|DB Performance]]
- [[Database Schema Design Simplified- Normalization vs Denormalization|Schema Design: Normalization vs Denormalization]]
- [[SQL vs NoSQL- Choosing the Right Database for An Application|SQL vs NoSQL]]
- [[Key Steps in the Database Selection Process|DB Selection Process]]
- [[Understanding Database Types|Understanding DB Types]]
- [[Unlocking the Power of SQL Queries for Improved Performance|SQL Query Performance]]
- [[A Crash Course on Relational Database Design|Relational DB Design]]
- [[Material view|Materialized Views]]

---

## ⚡ Scalability & Performance
- [[A Crash Course on Architectural Scalability|Architectural Scalability]]
- [[A Crash Course on Scaling the API Layer|Scaling the API Layer]]
- [[A Crash Course on Scaling the Data Layer|Scaling the Data Layer]]
- [[Top Scalability Strategies for Real-World Load|Top Scalability Strategies]]
- [[A Crash Course on Load Balancers for Scaling|Load Balancers for Scaling]]
- [[Understanding Load Balancers- Traffic Management at Scale|Load Balancers Deep Dive]]
- [[A Detailed Guide to Content Delivery Networks|CDN Guide]]
- [[A Guide to Rate Limiting Strategies|Rate Limiting Strategies]]
- [[A Guide to Top Caching Strategies|Caching Strategies]]
- [[Distributed Caching- The Secret to High-Performance Applications|Distributed Caching]]
- [[Stateless Architecture- The Key to Building Scalable and Resilient Systems|Stateless Architecture]]

---

## 🔐 Auth & Security
- [[Mastering Modern Authentication- Cookies, Sessions, JWT, and PASETO|Modern Authentication]]
- [[JWT vs PASETO- The Two Players of Token-Based Authentication|JWT vs PASETO]]
- [[EP176- How Does SSO Work|How Does SSO Work]]
- [[Oauth2|OAuth2]]
- [[How Grab Built An Authentication System for 180+ Million Users|Grab Auth System]]

---

## 🌍 Networking & Infrastructure
- [[A Crash Course in Networking|Networking Crash Course]]
- [[A Crash Course in IPv4 Addressing|IPv4 Addressing]]
- [[A Deep Dive into HTTP- From HTTP 1 to HTTP 3|HTTP 1→3 Deep Dive]]
- [[HTTP1 vs HTTP2 vs HTTP3 - A Deep Dive|HTTP Versions Comparison]]
- [[Infrastructure as Code|Infrastructure as Code]]
- [[Kubernetes Made Easy- A Beginner's Roadmap to Container Orchestration|Kubernetes Roadmap]]
- [[What are the differences between Virtualization (VMware) and Containerization (Docker)|Virtualization vs Containerization]]
- [[A Crash Course on Cell-based Architecture|Cell-based Architecture]]
- [[Why is a solid-state drive (SSD) fast|Why is SSD Fast]]

---

## 🏢 Case Studies (Real Systems)
- [[How Atlassian Migrated 4 Million Jira Databases to AWS Aurora|Atlassian: 4M Jira DBs → Aurora]]
- [[How Dropbox Built an AI Product Dash with RAG and AI Agents|Dropbox: AI Dash with RAG]]
- [[How Grab Built An Authentication System for 180+ Million Users|Grab: Auth at Scale]]
- [[How LinkedIn Scaled User Restriction System to 5 Million Queries Per Second|LinkedIn: 5M QPS Restriction]]
- [[How Reddit Delivers Notifications to Tens of Millions of Users|Reddit: Notifications at Scale]]
- [[How Slack Supports Billions of Daily Messages|Slack: Billions of Messages]]
- [[How Tinder's API Gateway Handles A Billion Swipes Per Day|Tinder: 1B Swipes/Day]]
- [[How Uber Eats Deduplicates Hundreds of Millions of Product Images|Uber Eats: Image Dedup]]
- [[How to avoid crawling duplicate URLs at Google scale|Google: URL Dedup at Scale]]
- [[Shopify Tech Stack|Shopify Tech Stack]]
- [[EP177- The Modern Software Stack|The Modern Software Stack]]

---

## 🔀 Concurrency
- [[Concurrency is NOT Parallelism|Concurrency is NOT Parallelism]]
- [[Understanding Message Queues|Understanding Message Queues]]
- [[Apache arvo, Protobuf|Apache Avro & Protobuf]]

---

## 💳 Domain-specific
- [[The Payments Ecosystem|The Payments Ecosystem]]
- [[Does Serverless Have Servers|Does Serverless Have Servers]]
- [[Why Executives Seem Out of Touch, and How to Reach Them|Why Executives Seem Out of Touch]]

---

## 🔗 Links vào hệ thống MOC
- [[MOC-System-Design]]
- [[MOC-Distributed-Systems]]
- [[MOC-Database]]
- [[MOC-Concurrency]]
