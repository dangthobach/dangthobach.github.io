import { useEffect, useRef } from 'react';
import './Hero.css';
import { useTypingAnimation } from '../../hooks/useTypingAnimation';
import profilePic from '../../assets/profile-pic.jpg';
import cvPdf from '../../assets/DangThoBach_EN.pdf';

const ROLES = [
  'Senior Java Engineer',
  'Backend Architect',
  'AI-Augmented Builder',
];

const Hero = () => {
  const typedText = useTypingAnimation(ROLES, 80, 2400);
  const glowRef = useRef(null);

  // Interactive mouse glow effect
  useEffect(() => {
    const handleMouseMove = (e) => {
      if (!glowRef.current) return;
      const x = (e.clientX / window.innerWidth) * 100;
      const y = (e.clientY / window.innerHeight) * 100;
      glowRef.current.style.background = `radial-gradient(700px circle at ${x}% ${y}%, rgba(0, 212, 255, 0.05), transparent 55%)`;
    };
    window.addEventListener('mousemove', handleMouseMove, { passive: true });
    return () => window.removeEventListener('mousemove', handleMouseMove);
  }, []);

  return (
    <section id="hero" className="hero">
      {/* Animated background */}
      <div className="hero__bg" aria-hidden="true">
        <div className="hero__blob hero__blob--1" />
        <div className="hero__blob hero__blob--2" />
        <div className="hero__grid" />
        <div className="hero__mouse-glow" ref={glowRef} />
      </div>

      <div className="hero__content">
        {/* ── Text side ── */}
        <div className="hero__text">
          <p className="hero__greeting">
            <span className="hero__greeting-line" aria-hidden="true" />
            Hello, I'm
          </p>

          <h1 className="hero__name">Dang Tho Bach</h1>

          <div className="hero__role" aria-live="polite">
            <span className="hero__role-prefix">I am a </span>
            <span className="hero__role-typed">{typedText}</span>
            <span className="hero__cursor" aria-hidden="true">|</span>
          </div>

          <p className="hero__bio">
            Senior Java Engineer with 7+ years building enterprise backend systems
            in Vietnamese fintech. Specializing in large-scale data processing, event-driven
            architectures, and AI-augmented engineering that ships 40% faster.
          </p>

          <div className="hero__cta">
            <a href={cvPdf} download="DangThoBach_CV.pdf" className="btn btn--primary" id="download-cv-btn">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
                <path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4" />
                <polyline points="7 10 12 15 17 10" />
                <line x1="12" y1="15" x2="12" y2="3" />
              </svg>
              Download CV
            </a>
            <a href="#contact" className="btn btn--secondary" id="contact-hero-btn">
              Get in Touch
            </a>
          </div>

          <div className="hero__socials">
            <a
              href="https://www.linkedin.com/in/bach201197/"
              target="_blank"
              rel="noopener noreferrer"
              className="hero__social-link"
              id="linkedin-link"
              aria-label="LinkedIn profile"
            >
              <svg viewBox="0 0 24 24" fill="currentColor" width="20" height="20" aria-hidden="true">
                <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
              </svg>
              LinkedIn
            </a>
            <a
              href="https://github.com/dangthobach"
              target="_blank"
              rel="noopener noreferrer"
              className="hero__social-link"
              id="github-link"
              aria-label="GitHub profile"
            >
              <svg viewBox="0 0 24 24" fill="currentColor" width="20" height="20" aria-hidden="true">
                <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
              </svg>
              GitHub
            </a>
          </div>
        </div>

        {/* ── Profile image ── */}
        <div className="hero__image-wrapper">
          <div className="hero__image-ring" aria-hidden="true" />
          <div className="hero__image-ring hero__image-ring--2" aria-hidden="true" />
          <img src={profilePic} alt="Dang Tho Bach — Senior Java Engineer" className="hero__image" />

          <div className="hero__badge hero__badge--top">
            <span className="hero__badge-dot" aria-hidden="true" />
            Open to senior roles
          </div>
          <div className="hero__badge hero__badge--bottom">
            <span>🏦</span>
            VPBank — Senior Java Engineer
          </div>
        </div>
      </div>

      {/* Scroll cue */}
      <div className="hero__scroll-cue" aria-hidden="true">
        <span>scroll</span>
        <div className="hero__scroll-line" />
      </div>
    </section>
  );
};

export default Hero;
